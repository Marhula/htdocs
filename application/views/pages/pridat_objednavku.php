...
<div class="form-group">
    <?php echo form_input(['name' => 'pocetKM', 'type' => 'number', 'placeholder' => 'Vzdialenost (km)', 'class' => 'form-control']);
    echo form_error('pocetKM', '<span class="badge badge-danger">', '</span>'); ?>
</div>
<div class="form-group">
    <?php echo form_input(['name' => 'cena', 'type' => 'number', 'placeholder' => 'Cena (€)', 'step' => '0.05', 'class' => 'form-control']);
    ...
    <?php if (count($zakaznici)): ?>
        ...
        <select class="form-control" name="zakaznik_ID" id="zakaznici">
            <?php foreach ($zakaznici as $zakaznik): ?>
                <option value="<?php echo $zakaznik->IDzakaznika; ?>"><?php echo $zakaznik->meno . " (ID: " . $zakaznik->IDzakaznika . ")"; ?></option>
            <?php endforeach; ?>
        </select>
    <?php else: ?>
        <div class="alert alert-danger">
...
<div class="form-group">
    <label for="vozidla">Stav:</label>
    <select class="form-control" name="stav" id="vozidla">
...
<div class="form-group">
    <div class="input-group date form_datetime " data-date="" data-date-format="yyyy-mm-dd hh:ii:00"
        ...
        <input class="form-control" size="16" type="text" name="kedy" placeholder="Deň a čas odvozu">
    </div>
</div>
